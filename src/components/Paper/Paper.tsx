...
const Paper = () => {
  ...
  const handleNoteClick = (id: number) => {
    console.log(id);
    getQuestions({ token, data: { question: id } })
      .then(({ data }) => {
        // TODO
        // 모달 열어서 데이터 전탈해야함!
      });
  ...
  return (
    <>
      ...
      <div className="px-5 flex flex-col items-center">
        ...
        <LastChatButton left="나의 질문" right="받은 질문" clicked="right" handleLeftClick={() => navigate('/pocket')} handleRightClick={() => navigate('/paper')} />
      </div>
    </>
  );
};
